...
export const createServiceReview = (serviceId, review) => async (
  dispatch,
  getState
) => {
  try {
    ...
    await axios.post(`/api/services/${serviceId}/reviews`, review, config)
...
export const listTopServices = () => async (dispatch) => {
  try {
    dispatch({ type: SERVICE_TOP_REQUEST })

    const { data } = await axios.get(`/api/services/top`)

    dispatch({
      type: SERVICE_TOP_SUCCESS,
      payload: data,
    })
  } catch (error) {
    dispatch({
      type: SERVICE_TOP_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
